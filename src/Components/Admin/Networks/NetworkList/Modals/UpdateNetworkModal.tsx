...
const UpdateNetworkModal: React.FC<UpdateNetworkModalProps> = ({
  isOpen,
  toggle,
  network,
  onSave,
  ...
  const onNext = async () => {
    ...
    // Local validation only for update flow; no server-side validation here
    setApiErrors({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!network) {
      toast.error("No network selected to update");
      return;
    }
    ...
    try {
      ...
      await onSave(network.slug, payload);
      ...
      const msg = getErrorMessage(error) || "Failed to update network";
      toast.error(msg);
    } finally {
      setIsSaving(false);
    }
  ...
  // Prefill form when opening for edit
  useEffect(() => {
    if (isOpen && network) {
      setFormData((prev) => ({
        network: {
          name: network.name || "",
          address: prev.network.address || "", // address may not exist on `Network` type
          primary_mobile: network.primary_mobile || "",
          email: network.email || "",
        },
        user: {
      ...
      setApiErrors({});
      setPhoneErrors({ primary_mobile: "", user_phone: "" });
  ...
  return (
    <Modal isOpen={isOpen} toggle={handleClose} size="lg" centered>
      <ModalHeader toggle={handleClose}>
        <h3 className="text-primary">Update Network</h3>
      </ModalHeader>
