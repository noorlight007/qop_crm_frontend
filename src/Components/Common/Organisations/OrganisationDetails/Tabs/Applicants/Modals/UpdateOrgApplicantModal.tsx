...
interface UpdateOrgApplicantModalProps {
  isOpen: boolean;
  toggle: () => void;
  applicantToUpdate: OrgLeadInfo | null;
}
...
const UpdateOrgApplicantModal: React.FC<UpdateOrgApplicantModalProps> = ({
  ...
  // Populate form when applicant data changes
  useEffect(() => {
  ...
  const handleUpdateApplicant = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  return (
    <Modal isOpen={isOpen} toggle={toggle} size="lg" centered>
      <ModalHeader toggle={toggle}>
        <span className="fs-4 text-primary">Update Applicant</span>
      </ModalHeader>
