...
const StampDutyCalculator: React.FC = () => {
  ...
  useEffect(() => {
    if (formData.propertyType === 'leasehold' && 
        formData.leaseStartDay && formData.leaseStartMonth && formData.leaseStartYear &&
        formData.leaseEndDay && formData.leaseEndMonth && formData.leaseEndYear) {
      ...
      try {
        ...
        // Validate lease term
        if (!term || isNaN(term.totalYears) || term.totalYears <= 0 || !isFinite(term.totalYears)) {
          return;
        }
        
        setLeaseTerm(term);
        
        // Initialize rent array based on lease years
        const yearsToCollect = Math.min(Math.ceil(term.totalYears), 5);
      ...
      formData.leaseEndDay, formData.leaseEndMonth, formData.leaseEndYear, formData.propertyType]);
  ...
  const calculateNPV = (yearlyRents: number[], totalYears: number): number => {
    // Validate inputs
    if (!yearlyRents || yearlyRents.length === 0 || isNaN(totalYears) || totalYears <= 0 || !isFinite(totalYears)) {
      return 0;
    }
    ...
    // Get actual rents for first 5 years (or lease duration if shorter)
    const yearsToUse = Math.min(Math.ceil(totalYears), 5);
    
    // Validate yearsToUse
    if (isNaN(yearsToUse) || yearsToUse < 1) {
      return 0;
    }
    
    // Calculate NPV for the years we have actual rent data
    ...
    // If lease > 5 years, use highest rent from first 5 years for remaining years
    if (totalYears > 5) {
      const validRents = yearlyRents.slice(0, yearsToUse).filter(r => isFinite(r) && r >= 0);
      if (validRents.length > 0) {
        const highestRent = Math.max(...validRents);
        const remainingYears = Math.ceil(totalYears) - yearsToUse;
  ...
  const calculateSDLT = () => {
    ...
    const { propertyUse, propertyType, isNonUKResident, isPurchasingAsIndividual, 
            willOwnMultipleProperties, isReplacingMainResidence } = formData;
    ...
    // CRITICAL: If replacing main residence, NO surcharges apply at all
    const applySurcharges = isReplacingMainResidence !== true;
    
    // Determine surcharges
    const isAdditionalProperty = applySurcharges && 
                                  isPurchasingAsIndividual === true && 
                                  willOwnMultipleProperties === true;
    const applyNonResidentSurcharge = applySurcharges && isNonUKResident === true;

    // Calculate premium/purchase price tax
    if (propertyUse === 'residential') {
      // Determine rates based on property type (Freehold vs Leasehold)
      const rates = propertyType === 'leasehold' 
        ? [
            // Leasehold premium rates (from April 1, 2025)
        ...
        : [
            // Freehold rates (from April 1, 2025)
            { min: 0, max: 125000, rate: 0 },
            { min: 125001, max: 250000, rate: 2 },
            { min: 250001, max: 925000, rate: 5 },
            { min: 925001, max: 1500000, rate: 10 },
      ...
      for (const band of rates) {
        if (price > band.min) {
          const taxableAmount = Math.min(price, band.max) - Math.max(band.min - 1, 0);
          if (taxableAmount > 0) {
            let effectiveRate = band.rate;
            
            // Add surcharges
            if (isAdditionalProperty) effectiveRate += 5;
            if (applyNonResidentSurcharge) effectiveRate += 2;
    ...
    if (propertyType === 'leasehold' && leaseTerm && leaseTerm.totalYears > 0) {
      const yearlyRentValues = formData.yearlyRents.map(r => parseFloat(r.replace(/,/g, '')) || 0);
      rentNPV = calculateNPV(yearlyRentValues, leaseTerm.totalYears);

      if (propertyUse === 'residential') {
        // Residential leasehold rent threshold: £125,000
        if (rentNPV > 125000) {
          const taxableRent = rentNPV - 125000;
          let rentRate = 1;
          // Non-UK resident surcharge on rent (only if not replacing main residence)
          if (applyNonResidentSurcharge) rentRate += 2;
        ...
        // Non-residential leasehold rent thresholds: £150,000 and £5,000,000
        if (rentNPV > 150000) {
        ...
        if (formData.isPurchasingAsIndividual === true) {
          ...
          if (formData.willOwnMultipleProperties === true) {
            ...
            if (formData.hasEverOwnedProperty === true) {
              // If they've owned before, ask about shared ownership if leasehold
              if (formData.propertyType === 'leasehold') {
              ...
              // After main residence question, ask shared ownership if leasehold
              if (formData.propertyType === 'leasehold') {
                steps++; // Shared ownership question
                if (formData.isSharedOwnership === true) {
                  steps++; // Market value question
  ...
  const validateCurrentStep = (): boolean => {
    ...
    if (currentStep === stepCounter) {
      if (!formData.effectiveDay || !formData.effectiveMonth || !formData.effectiveYear) {
      ...
      if (formData.isNonUKResident !== null) {
        ...
        if (formData.isPurchasingAsIndividual === true) {
          ...
          if (formData.willOwnMultipleProperties === true) {
            ...
            // NEW: Ask about previous ownership
            if (currentStep === stepCounter) {
            ...
            // Shared ownership questions (for leasehold only)
            if (formData.propertyType === 'leasehold') {
              if (currentStep === stepCounter) {
              ...
              if (formData.isSharedOwnership === true) {
                if (currentStep === stepCounter) {
                  if (!formData.sharedMarketValueOption) {
                    newErrors.sharedMarketValueOption = 'Please select market value option';
                  }
                }
                stepCounter++;
                ...
                if (formData.sharedMarketValueOption === 'lte500k') {
                  if (currentStep === stepCounter) {
                    if (!formData.sharedMarketValueElection) {
                      newErrors.sharedMarketValueElection = 'Please select an election';
                    }
                  }
                  stepCounter++;
                }
    ...
    if (formData.propertyType === 'leasehold') {
      // Lease Start Date
      if (currentStep === stepCounter) {
        if (!formData.leaseStartDay || !formData.leaseStartMonth || !formData.leaseStartYear) {
      ...
      // Lease End Date
      if (currentStep === stepCounter) {
        if (!formData.leaseEndDay || !formData.leaseEndMonth || !formData.leaseEndYear) {
          ...
          if (endDay < 1 || endDay > 31 || endMonth < 1 || endMonth > 12 || endYear < 1900 || endYear > 2100) {
    ...
    if (formData.propertyType === 'leasehold' && currentStep === stepCounter) {
      if (!leaseTerm || !leaseTerm.totalYears || leaseTerm.totalYears <= 0) {
        newErrors.yearlyRents = 'Please ensure you have entered valid lease dates';
      } else {
  ...
  const renderStep = () => {
            ...
            <FormGroup check className="my-3 mx-3">
              <Label check>
                <Input
                  ...
                  onChange={() => setFormData({ ...formData, propertyType: 'freehold' })}
                />
                {' '}Freehold
              </Label>
            </FormGroup>
            <FormGroup check className="my-3 mx-3">
              <Label check>
                <Input
                  ...
                  onChange={() => setFormData({ ...formData, propertyType: 'leasehold' })}
                />
                {' '}Leasehold
              </Label>
            </FormGroup>
          </FormGroup>
          {errors.propertyType && <Alert color="danger">{errors.propertyType}</Alert>}
        </div>
      );
    }
    stepCounter++;
    ...
    if (currentStep === stepCounter) {
      return (
        <div>
          <h5 className="mb-4">Is the transaction residential or non-residential?</h5>
          <p className="text-muted small mb-3">If it's a mixed transaction, choose 'Non-residential'.</p>
          <FormGroup tag="fieldset">
            <FormGroup check className="my-3 mx-3">
              <Label check>
                <Input
                  ...
                  onChange={() => setFormData({ ...formData, propertyUse: 'residential' })}
                />
                {' '}Residential
              </Label>
            </FormGroup>
            <FormGroup check className="my-3 mx-3">
              <Label check>
                <Input
                  ...
                  onChange={() => setFormData({ ...formData, propertyUse: 'non-residential' })}
                />
                {' '}Non-residential
              </Label>
            </FormGroup>
          </FormGroup>
          {errors.propertyUse && <Alert color="danger">{errors.propertyUse}</Alert>}
        </div>
      );
    }
    stepCounter++;
    ...
    if (currentStep === stepCounter) {
      return (
        <div>
          ...
          <p className="text-muted small mb-3">This is usually the completion date.</p>
          ...
          {errors.effectiveDate && <Alert color="danger">{errors.effectiveDate}</Alert>}
        </div>
      );
    }
    stepCounter++;
    ...
    if (formData.propertyUse === 'residential') {
              ...
              <FormGroup check className="my-3 mx-3">
                <Label check>
                  <Input
                    ...
                    onChange={() => setFormData({ ...formData, isNonUKResident: true })}
                  />
                  {' '}Yes
                </Label>
              </FormGroup>
              <FormGroup check className="my-3 mx-3">
                <Label check>
                  <Input
                    ...
                    onChange={() => setFormData({ ...formData, isNonUKResident: false })}
                  />
                  {' '}No
                </Label>
              </FormGroup>
            </FormGroup>
            {errors.isNonUKResident && <Alert color="danger">{errors.isNonUKResident}</Alert>}
          </div>
        );
      }
      stepCounter++;
      ...
      if (currentStep === stepCounter) {
        return (
          <div>
            <h5 className="mb-3">Are you purchasing the property as an individual?</h5>
            <p className="text-muted small mb-3">
              Choose 'Yes' if you're buying on your own, or you're married or in a civil partnership. 
              Choose 'No' if you're a company or trust.
            </p>
            <FormGroup tag="fieldset">
              <FormGroup check className="my-3 mx-3">
                <Label check>
                  <Input
                    ...
                    onChange={() => setFormData({ ...formData, isPurchasingAsIndividual: true })}
                  />
                  {' '}Yes
                </Label>
              </FormGroup>
              <FormGroup check className="my-3 mx-3">
                <Label check>
                  <Input
                    ...
                    onChange={() => setFormData({ ...formData, isPurchasingAsIndividual: false })}
                  />
                  {' '}No
                </Label>
              </FormGroup>
            </FormGroup>
            {errors.isPurchasingAsIndividual && <Alert color="danger">{errors.isPurchasingAsIndividual}</Alert>}
          </div>
        );
      }
      stepCounter++;
      ...
      if (formData.isPurchasingAsIndividual === true) {
        if (currentStep === stepCounter) {
          return (
            <div>
              <h5 className="mb-4">Will the purchase of the property result in owning two or more properties?</h5>
              <FormGroup tag="fieldset">
                <FormGroup check className="my-3 mx-3">
                  <Label check>
                    <Input
                      ...
                      onChange={() => setFormData({ ...formData, willOwnMultipleProperties: true })}
                    />
                    {' '}Yes
                  </Label>
                </FormGroup>
                <FormGroup check className="my-3 mx-3">
                  <Label check>
                    <Input
                      ...
                      onChange={() => setFormData({ ...formData, willOwnMultipleProperties: false })}
                    />
                    {' '}No
                  </Label>
                </FormGroup>
              </FormGroup>
              {errors.willOwnMultipleProperties && <Alert color="danger">{errors.willOwnMultipleProperties}</Alert>}
            </div>
          );
        }
        stepCounter++;
        ...
        if (formData.willOwnMultipleProperties === true) {
          if (currentStep === stepCounter) {
            return (
              <div>
                <h5 className="mb-3">Is the property being purchased replacing your main residence?</h5>
                <p className="text-muted small mb-3">
                  ...
                  <FormGroup check className="my-3 mx-3">
                    <Label check>
                      <Input
                        ...
                        onChange={() => setFormData({ ...formData, isReplacingMainResidence: true })}
                      />
                      {' '}Yes
                    </Label>
                  </FormGroup>
                  <FormGroup check className="my-3 mx-3">
                    <Label check>
                      <Input
                        ...
                        onChange={() => setFormData({ ...formData, isReplacingMainResidence: false })}
                      />
                      {' '}No
                    </Label>
                  </FormGroup>
                </FormGroup>
                {errors.isReplacingMainResidence && <Alert color="danger">{errors.isReplacingMainResidence}</Alert>}
              </div>
            );
          }
          stepCounter++;
          ...
          if (currentStep === stepCounter) {
            return (
              <div>
                <h5 className="mb-3">Have you ever owned or part owned another property?</h5>
                <p className="text-muted small mb-3">
                  ...
                  <FormGroup check className="my-3 mx-3">
                    <Label check>
                      <Input
                        ...
                        onChange={() => setFormData({ ...formData, hasEverOwnedProperty: true })}
                      />
                      {' '}Yes
                    </Label>
                  </FormGroup>
                  <FormGroup check className="my-3 mx-3">
                    <Label check>
                      <Input
                        ...
                        onChange={() => setFormData({ ...formData, hasEverOwnedProperty: false })}
                      />
                      {' '}No
                    </Label>
                  </FormGroup>
                </FormGroup>
                {errors.hasEverOwnedProperty && <Alert color="danger">{errors.hasEverOwnedProperty}</Alert>}
              </div>
            );
          }
          stepCounter++;
          ...
          if (formData.hasEverOwnedProperty === false) {
            if (currentStep === stepCounter) {
              return (
                <div>
                  <h5 className="mb-4">Will this property be your main residence?</h5>
                  <FormGroup tag="fieldset">
                    <FormGroup check className="my-3 mx-3">
                      <Label check>
                        <Input
                          ...
                          onChange={() => setFormData({ ...formData, willThisBeMainResidence: true })}
                        />
                        {' '}Yes
                      </Label>
                    </FormGroup>
                    <FormGroup check className="my-3 mx-3">
                      <Label check>
                        <Input
                          ...
                          onChange={() => setFormData({ ...formData, willThisBeMainResidence: false })}
                        />
                        {' '}No
                      </Label>
                    </FormGroup>
                  </FormGroup>
                  {errors.willThisBeMainResidence && <Alert color="danger">{errors.willThisBeMainResidence}</Alert>}
                </div>
              );
            }
            stepCounter++;
          ...
          // Shared ownership questions - for leasehold residential when not owning multiple
          if (formData.propertyType === 'leasehold') {
            if (currentStep === stepCounter) {
              return (
                <div>
                  <h5 className="mb-3">Are you buying the property through a shared ownership scheme?</h5>
                  <FormGroup tag="fieldset">
                    <FormGroup check className="my-3 mx-3">
                      <Label check>
                        <Input
                          ...
                          onChange={() => setFormData({ ...formData, isSharedOwnership: true })}
                        />
                        {' '}Yes
                      </Label>
                    </FormGroup>
                    <FormGroup check className="my-3 mx-3">
                      <Label check>
                        <Input
                          ...
                          onChange={() => setFormData({ ...formData, isSharedOwnership: false })}
                        />
                        {' '}No
                      </Label>
                    </FormGroup>
                  </FormGroup>
                  {errors.isSharedOwnership && <Alert color="danger">{errors.isSharedOwnership}</Alert>}
                </div>
              );
            }
            stepCounter++;
            ...
            if (formData.isSharedOwnership === true) {
              if (currentStep === stepCounter) {
                return (
                  <div>
                    <h5 className="mb-3">What is the market value of the property?</h5>
                    <FormGroup tag="fieldset">
                      <FormGroup check className="my-3 mx-3">
                        <Label check>
                          <Input
                            ...
                            checked={formData.sharedMarketValueOption === 'lte500k'}
                            onChange={() => setFormData({ ...formData, sharedMarketValueOption: 'lte500k' })}
                          />
                          {' '}£500,000 or less
                        </Label>
                      </FormGroup>
                      <FormGroup check className="my-3 mx-3">
                        <Label check>
                          <Input
                            type="radio"
                            name="sharedMarketValueOption"
                            checked={formData.sharedMarketValueOption === 'gt500k'}
                            onChange={() => setFormData({ ...formData, sharedMarketValueOption: 'gt500k' })}
                          />
                          {' '}More than £500,000
                        </Label>
                      </FormGroup>
                    </FormGroup>
                    {errors.sharedMarketValueOption && <Alert color="danger">{errors.sharedMarketValueOption}</Alert>}
                  </div>
                );
              }
              stepCounter++;
              ...
              if (formData.sharedMarketValueOption === 'lte500k') {
                if (currentStep === stepCounter) {
                  return (
                    <div>
                      <h5 className="mb-3">Do you want to pay SDLT on the market value or only on the percentage of the property you are buying?</h5>
                      <FormGroup tag="fieldset">
                        <FormGroup check className="my-3 mx-3">
                          <Label check>
                            <Input
                              ...
                              checked={formData.sharedMarketValueElection === 'market'}
                              onChange={() => setFormData({ ...formData, sharedMarketValueElection: 'market' })}
                            />
                        ...
                        <FormGroup check className="my-3 mx-3">
                          <Label check>
                            <Input
                              type="radio"
                              name="sharedMarketValueElection"
                              checked={formData.sharedMarketValueElection === 'stages'}
                              onChange={() => setFormData({ ...formData, sharedMarketValueElection: 'stages' })}
                            />
                            {' '}Pay SDLT in stages
                          </Label>
                        </FormGroup>
                      </FormGroup>
                      {errors.sharedMarketValueElection && <Alert color="danger">{errors.sharedMarketValueElection}</Alert>}
                    </div>
                  );
                }
                stepCounter++;
      ...
      if (currentStep === stepCounter) {
        return (
          <div>
            ...
            <p className="text-muted small mb-3">This is the date your term starts.</p>
            <p className="text-muted small mb-3">For example, 31 3 2014.</p>
            ...
            {errors.leaseStartDate && <Alert color="danger">{errors.leaseStartDate}</Alert>}
          </div>
        );
      }
      stepCounter++;
      ...
      if (currentStep === stepCounter) {
        return (
          <div>
            ...
            <p className="text-muted small mb-3">This is the date your term ends.</p>
            ...
            {leaseTerm && (
              <Alert color="info" className="mt-3">
                <strong>Term of lease:</strong> {leaseTerm.years} years {leaseTerm.days} days
              </Alert>
            )}
            {errors.leaseEndDate && <Alert color="danger">{errors.leaseEndDate}</Alert>}
          </div>
        );
      }
      stepCounter++;
    ...
    if (currentStep === stepCounter) {
      return (
        <div>
          ...
          <p className="text-muted small mb-3">This is the chargeable consideration.</p>
          ...
          {errors.purchasePrice && <Alert color="danger">{errors.purchasePrice}</Alert>}
        </div>
      );
    }
    stepCounter++;
    ...
    if (formData.propertyType === 'leasehold' && currentStep === stepCounter) {
      ...
      if (!leaseTerm || !leaseTerm.totalYears || leaseTerm.totalYears <= 0) {
        return (
          <div>
            <Alert color="warning">
              Please ensure you have entered valid lease start and end dates in the previous steps.
            </Alert>
          </div>
        );
      }
      ...
      return (
        <div>
          ...
          <Alert color="info" className="mb-3">
            <small>
              <strong>Note:</strong> Enter the rent for each year of the lease (up to 5 years). 
              {leaseTerm.totalYears > 5 && ' The highest rent from the first 5 years will be used for remaining years.'}
            </small>
          </Alert>
          ...
          {leaseTerm.totalYears > 5 && (
            <Alert color="warning" className="mt-3">
              <small>
                <strong>Highest rent from years 1-5 will be used for years 6-{Math.ceil(leaseTerm.totalYears)}</strong>
              </small>
            </Alert>
          )}
          {errors.yearlyRents && <Alert color="danger">{errors.yearlyRents}</Alert>}
        </div>
      );
    }
          ...
          <Col>
            <Card>
              ...
              <CardBody>
                {/* Summary */}
                <div className="mb-4">
                  <h4 className="mb-3">Results of calculation based on SDLT rules for the effective date entered</h4>
                  <Card className="border-success">
                    <CardBody>
                      <h5 className="mb-3">Result of SDLT calculation</h5>
                      
                      <div className="mb-4 pb-3 border-bottom">
                        <div className="text-muted small mb-1">Total amount of tax for this transaction</div>
                      ...
                      {formData.propertyType === 'freehold' && (
                        <Row>
                          ...
                          <Col xs={4} className="text-end">
                            {formatCurrency(parseFloat(formData.purchasePrice.replace(/,/g, '')))}
                          </Col>
                        </Row>
                      )}
                    </CardBody>
                ...
                <div className="mb-4">
                  ...
                  <div className="d-flex gap-2 flex-wrap mb-2">
                    <Badge color="primary">{formData.propertyType === 'freehold' ? 'Freehold' : 'Leasehold'}</Badge>
                    <Badge color="primary">
                      {formData.propertyUse === 'residential' ? 'Residential' : 'Non-Residential'}
                    </Badge>
                    {formData.isNonUKResident && !formData.isReplacingMainResidence && (
                    ...
                    {formData.isReplacingMainResidence === true && (
                      <Badge color="info">Replacing Main Residence (No Surcharges)</Badge>
                    )}
                  </div>
                ...
                <h5 className="mb-3">SDLT on {formData.propertyType === 'leasehold' ? 'premium' : 'purchase price'}</h5>
                <Table bordered responsive hover className="mb-4">
                  <thead className="table-light">
                    <tr>
                      <th>{formData.propertyType === 'leasehold' ? 'Premium' : 'Purchase price'} bands (£)</th>
                  ...
                  <tbody>
                    ...
                    <tr className="table-light fw-bold">
                      <td colSpan={2}>SDLT due on the {formData.propertyType === 'leasehold' ? 'premium' : 'purchase price'}</td>
                ...
                {formData.propertyType === 'leasehold' && calculationResult.rentBreakdown.length > 0 && (
                  <>
                    <h5 className="mb-3">SDLT on rent</h5>
                    <Alert color="info" className="mb-3">
                      <strong>Net present value:</strong> {formatCurrency(calculationResult.rentNPV)}
                    </Alert>
                    <Table bordered responsive hover className="mb-4">
                      <thead className="table-light">
                        <tr>
                ...
                <Alert color="warning" className="mt-4">
                  ...
                  <ul className="mb-0 small">
                    <li>This is an estimate only. Your solicitor will calculate the exact SDLT due.</li>
                    <li>SDLT must be paid within 14 days of completion.</li>
                    <li>Rates are based on transactions from 1 April 2025 onwards.</li>
                    {formData.isReplacingMainResidence === true && (
                      <li className="text-info">
                        <strong>Replacing main residence:</strong> Additional property and non-UK resident surcharges do not apply. If you haven't sold your previous home yet, you may claim a refund if it's sold within 3 years.
                      </li>
                    )}
                  </ul>
                </Alert>
        ...
        <Col>
            ...
            <CardBody>
              ...
              <div className="mb-4">
                <div className="d-flex justify-content-between mb-2">
                  <span className="small">Step {currentStep} of {totalSteps}</span>
                  <span className="small">{Math.round(progressPercentage)}% Complete</span>
                </div>
          ...
          <Card className="mt-3 border-0">
            <CardBody className="small">
              ...
              <p className="mb-0">
                Stamp Duty Land Tax (SDLT) is a tax paid when purchasing property in England and Northern Ireland. 
                The amount depends on the property price, type, and your circumstances.
              </p>
            </CardBody>
          </Card>
        </Col>
