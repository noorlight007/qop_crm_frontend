...
const initialForm: EditAdFormState = {
  title: "",
  redirect_url: "",
...
const getInitialForm = (adData: AdvertiserAdsData | null): EditAdFormState => ({
  title: adData?.title ?? "",
  redirect_url: adData?.redirect_url ?? "",
  placement: adData?.placement ?? "DASHBOARD_TOP",
  start_date: adData?.start_date
    ? toLocalDateTimeString(adData.start_date)
    : "",
  end_date: adData?.end_date ? toLocalDateTimeString(adData.end_date) : "",
  imageFile: null,
...
const EditAdModal: React.FC<EditAdModalProps> = ({
  ...
  const [editAd, { isLoading }] = useEditAdvertiserAdMutation();
  const [formData, setFormData] = useState<EditAdFormState>(initialForm);
  ...
  useEffect(() => {
    if (!isOpen) return;
    setFormData(getInitialForm(adData));
    setTouched({
      title: false,
      redirect_url: false,
      placement: false,
  ...
  const errors = useMemo(() => {
    ...
    if (!adData?.alias) {
      next.adData = "Missing ad information.";
    }
    ...
    const hasExistingImage = Boolean(adData?.image);
    ...
    return next;
  }, [advertiserAlias, adData, formData]);

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    const nextValue = name === "is_active" ? value === "true" : value;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!advertiserAlias || !adData?.alias) {
      setSubmitError("Missing advertiser or ad information.");
      return;
    }
    ...
    if (!startIso || !endIso) {
      setSubmitError("Invalid date values.");
      return;
    }
