import { useUpdateOtherOccupantMutation } from "@/Redux/Reducers/CommonComponents/SingleCaseInfo/CaseDetails/SecurityProperty/OtherOccupantsApi";
...
const UpdateOtherOccupantModal: React.FC<OtherOccupantModalProps> = ({
  isOpen,
  toggle,
  selectedOccupant,
}) => {
  const { casealias } = useParams();
  const [updateOtherOccupant, { isLoading }] = useUpdateOtherOccupantMutation();
  ...
  useEffect(() => {
    if (selectedOccupant) {
      setFormData({
        full_name: selectedOccupant.full_name,
        date_of_birth: selectedOccupant.date_of_birth || "",
        relationship: selectedOccupant.relationship || "",
      });
    }
  }, [selectedOccupant]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const serverMsg =
        extractErrorMessage(err) || "Failed to update other occupant";
...
export default UpdateOtherOccupantModal;
