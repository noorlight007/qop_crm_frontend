...
const refreshAccessToken = async (): Promise<string | null> => {
  ...
  refreshPromise = (async () => {
    try {
      ...
      // If no refresh token found, we can't refresh - return null without logging out
      // This can happen on first login or in legitimate auth scenarios
      if (!refreshToken) {
      ...
      if (!response.ok) {
        // Refresh failed - token is invalid, so logout
